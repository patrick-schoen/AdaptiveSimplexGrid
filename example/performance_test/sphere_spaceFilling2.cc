...
void circle_test(Grid &subgrid, Communicator &comm, Decomposition &subDecomposition, GlobalId &subGlobalId )
{
    static const int NV = Grid::NV;
    static const int DIM = Grid::dim;
    
    printf("circle_test with %d ranks \n",(int)comm.size());
    
    double nE_fine = 1E06 ;
    double nE_coarse = 1E06 / 8. ;
    ...
    std::ofstream ofs_refine, ofs_coarse, ofs_repartition, ofs_repartition_coarse,ofs_parmetis; 
    ...
    std::string file_repartition_coarse = "out/repartition_coarse_" + std::to_string(nRanks) + ".txt";
    std::string file_parmetis = "out/parmetis" + std::to_string(nRanks) + ".txt";
    ...
    ofs_repartition_coarse.open(file_repartition_coarse, std::ofstream::out | std::ofstream::trunc);
    ...
    {        
        ...
        double Time_local1 = 0.;
        double Time_average1 = 0.;
        double Time_max1 = 0.;
 
        double Time_local2 = 0.;
        double Time_average2 = 0.;
        double Time_max2 = 0.;
        ...
        while( nE_global < nE_fine ) {
            ...
            double time_local,time_average,time_max;
            timer.begin();
            refine_parallel(subDecomposition,subgrid,marked,curve);
	    ...
	    timer.getTime(time_local1,time_average1,time_max1);
	    Time_local1 += time_local1,
            Time_average1 += time_average1,
            Time_max1 += time_max1;

            double time_local2,time_average2,time_max2;            
	    timer.begin();
            subGlobalId.communicate();
            redistributeNewPartition_timer( subgrid, comm, subDecomposition, subGlobalId, partition,curve);
            timer.getTime(time_local2,time_average2,time_max2);
            Time_local2 += time_local2,
            Time_average2 += time_average2,
            Time_max2 += time_max2;
        ...
        if( comm.master() ){
            ofs_refine << j << "," << Time_local << "," << Time_average << "," << Time_max << "," << counter << "," << nC_global << "," << nE_global << std::endl; 
            ofs_refine.flush();
        }
        
        if( comm.master() ){
           ofs_parmetis << j << "," << Time_local1 << "," << Time_average1 << "," << Time_max1 << "," << counter << "," << nC_global << "," << nE_global << std::endl; 
           ofs_parmetis.flush();
        }
        
        if( comm.master() ){
            ofs_repartition << j << "," << Time_local2 << "," << Time_average2 << "," << Time_max2 << "," << counter << "," << nC_global << "," << nE_global << std::endl; 
...
#if 0    
        u = u_0_circle(subgrid.coordinates, Y, radius );
        std::string file_name = "vtu/circle_";
	...
	Time_local1 = 0.;
        ...
        while (nE_global > nE_coarse) {
            ...
            double time_local1,time_average1,time_max1;
            timer.begin();
            repartitionForCoarsening( subgrid, comm, subDecomposition, subGlobalId, marked,curve );
            timer.getTime(time_local1,time_average1,time_max1);
            ...
            double time_local,time_average,time_max;
            timer.begin();
            coarse_parallel(subDecomposition,subgrid,marked,curve);
        ...
        if( comm.master() ){
            ofs_coarse << j << "," << Time_local << "," << Time_average << "," << Time_max << "," << counter << "," << nC_global << "," << nE_global << std::endl; 
            ofs_coarse.flush();
            ofs_repartition_coarse << j << "," << Time_local1 << "," << Time_average1 << "," << Time_max1 << "," << "," << counter << "," << nC_global << "," << nE_global << std::endl; 
            ofs_repartition_coarse.flush();
        }
    ...
    ofs_repartition_coarse.close();   
}

template< class Grid, class Communicator, class Decomposition, class GlobalId, class SpaceFillingCurve >
void circle_test_local(Grid &subgrid, Communicator &comm, Decomposition &subDecomposition, GlobalId &subGlobalId, SpaceFillingCurve &curve )
{
    static const int NV = Grid::NV;
    static const int DIM = Grid::dim;
    
    if(comm.master()) printf("circle_test with %d ranks \n",(int)comm.size());
...
#endif
    circle_test_local(subgrid, comm, subDecomposition, subGlobalId, curve_ );
