...
void circle_test_local(Grid &subgrid, Communicator &comm, Decomposition &subDecomposition, GlobalId &subGlobalId, SpaceFillingCurve &curve )
{
    ...
    if(comm.master()) printf("circle_test with %d ranks, space filling curve \n",(int)comm.size());
    
    double nE_fine = (double)comm.size() * 1E05 * 1.3;
    double nE_coarse = (double)comm.size() * 1E05 / 8.;
    ...
    std::vector<double> refine_time, coarse_time, repartition_time, parmetis_time, marking_time, nE_global_vec, nC_global_vec, numberIterations_refine, numberIterations_coarse, nE_global_vec_coarse, nC_global_vec_coarse;
    ...
    {        
        ...
        while( nE_global < nE_fine ) {
            ...
            timer.begin();
            refine_parallel_markingTime(markingTime,subDecomposition,subgrid,marked,curve);
        ...
        timer.begin();
        //spaceFillingCurvePartition(comm, subgrid, curve, partition); 
        ...
        redistributeNewPartition( subgrid, comm, subDecomposition, subGlobalId, partition,curve);
        ...
        std::string file_name = "vtu/subgrid_";
        ...
        v = u_0_circle(subgrid.coordinates, Y, radius);
        export2vtk(subgrid,v,partition,partition,file_name.c_str());  
#endif             
        
        if(comm.master()) printf("nElements = %d nC = %d, nIterations = %d, time refine = %lf (%lf), time metis = %lf, time repart = %lf \n", nE_global, nC_global, nIterations_ref, Time_refine, Time_marking, Time_parMetis, Time_repart);
        ...
        nE_global_vec_coarse.push_back(nE_global);
        nC_global_vec_coarse.push_back(nC_global);
    ...
    std::string file_refine = "out_weak/refine_" + std::to_string(nRanks) + ".txt";
    std::string file_coarse = "out_weak/coarse_" + std::to_string(nRanks) + ".txt";
    std::string file_repartition = "out_weak/repartition_" + std::to_string(nRanks) + ".txt";
    std::string file_parmetis = "out_weak/parmetis_" + std::to_string(nRanks) + ".txt";
    std::string file_marking = "out_weak/marking_" + std::to_string(nRanks) + ".txt";
    ...
    for(int k = 0; k < coarse_time.size(); k++){
        ...
        double time_average = time_global / subDecomposition.size();
        if( comm.master() ){
            ofs_coarse << k << "," << time_average << "," << time_max << "," << nC_global_vec_coarse[k] << "," << nE_global_vec_coarse[k] << "," << numberIterations_coarse[k] << std::endl; 
            ofs_coarse.flush();
        }  
    }
    ...
    for(int k = 0; k < repartition_time.size(); k++){
        ...
        double time_average = time_global / subDecomposition.size();
        if( comm.master() ){
            ofs_repartition << k << "," << time_average << "," << time_max << "," << nC_global_vec[k] << "," << nE_global_vec[k] << std::endl;  
