...
void circle_test_local(Grid &subgrid, Communicator &comm, Decomposition &subDecomposition, GlobalId &subGlobalId, SpaceFillingCurve &curve )
{
    ...
    if(comm.master()) printf("circle_test with %d ranks, space Filling\n",(int)comm.size());
    ...
    {        
        ...
        while( nE_global < nE_fine ) {
            ...
            timer.begin();
            redistributeNewPartition_Timer( subgrid, comm, subDecomposition, subGlobalId, partition, curve, time_repart_data, time_repart_communcation);    
            ...
            std::string file_name = "vtu/refine";
